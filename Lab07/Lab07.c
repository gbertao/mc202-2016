...
int main (void) {
	...
	CACHE *cache;
	int *entrada;
	int n, a, c, i, arq, r, rid, prAtual = 20000, solucao = 0, cont = 0, j;
	...
	for(i = 0; i < a; i++) {
		r = procurar_lista(cache, entrada[i]);

		if(r >= cache->tamanho) {//Se o arquivo nao for encontrado na cache
			if(cache->tamanho >= c) {//cache cheia, encontrar qual remover da cache
				for(j = i + 1; j < a; j++) {
					rid = procurar_lista(cache, entrada[j]);
					if(rid < cache->tamanho) {
						if(cont >= cache->tamanho - 1) {
							inserir_lista(cache, entrada[j], rid, &prAtual);
							descer(cache, 0);
							cont = 0;
						} else {
							cont++;
				...
				inserir_lista(cache, entrada[i], r, &prAtual);//cache vazia
			}
			solucao++;
		} else {
